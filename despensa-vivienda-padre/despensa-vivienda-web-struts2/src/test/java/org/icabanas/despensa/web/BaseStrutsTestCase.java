package org.icabanas.despensa.web;
...
public abstract class BaseStrutsTestCase {
    private static final String CONFIG_LOCATIONS = "classpath:applicacionContext-action-test.xml";
    ...
    protected <T> T createAction(Class<T> clazz, String namespace, String name)
        ...
        return createAction(clazz, namespace, name, new HashMap());
    }
    
    protected <T> T createAction(Class<T> clazz,
			String namespace, String name, Map parametros) throws Exception {
        ...
        proxy = dispatcher.getContainer().getInstance(ActionProxyFactory.class).
                createActionProxy(
                namespace, name, null, null, true, false);
        ...
        request.getSession().setAttribute(IParametros.NUMERO_REGISTROS_POR_PAGINA, Integer.valueOf(2));
    ...
    @BeforeClass
    public static void configuraTest() throws Exception {
        if( applicationContext == null ) {
            // this is the first time so initialize Spring context
            servletContext = new MockServletContext();
            servletContext.addInitParameter(ContextLoader.CONFIG_LOCATION_PARAM,
                    CONFIG_LOCATIONS);
//            servletContext.setAttribute(IParametros.NUMERO_REGISTROS_POR_PAGINA, 2);            
